...
// If you don't have access, we'll use a fallback approach
...
try {
  ...
  // SplitText not available, we'll use a fallback
...
const SplitTextComponent: React.FC<SplitTextProps> = ({
  ...
  useGSAP(
    () => {
      ...
      // Use GSAP SplitText if available
      const startPct = (1 - threshold) * 100;
      const marginMatch = /^(-?\d+(?:\.\d+)?)(px|em|rem|%)?$/.exec(rootMargin);
      const marginValue = marginMatch ? parseFloat(marginMatch[1]) : 0;
      const marginUnit = marginMatch ? marginMatch[2] || 'px' : 'px';
  ...
  const renderTag = () => {
    ...
    const style: React.CSSProperties = {
      ...(isInline ? { verticalAlign: 'baseline' } : { textAlign }), // Fix inline-block spacing
      wordWrap: 'break-word',
      willChange: 'transform, opacity'
    };

    // For span tags, use inline-block with vertical-align to preserve natural spacing
    const classes = `split-parent ${isInline ? 'inline-block' : 'inline-block'} overflow-hidden whitespace-normal ${className}`;
